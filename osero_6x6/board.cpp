...
CellPosition parseCellPosition(const std::string& str)
{
	...
	char col = str.at(0);
	char row = str.at(1);
	const char col_max = static_cast<char>('a' + Board::WIDTH - 1);
	const char row_max = static_cast<char>('1' + Board::HEIGHT - 1);
	if (col < 'a' || col > col_max || row < '1' || row > row_max) {
		throw std::invalid_argument{"invalid format \"" + str + "\""};
	}
	int x = col - 'a';
	int y = row - '1';
...
{
	// 6x6 の中央 2x2 を初期配置にする
	const int c = WIDTH / 2;  // = 3
	set({c - 1, c - 1}, CellState::WHITE);  // (2, 2)
	set({c - 1, c}, CellState::BLACK);      // (2, 3)
	set({c, c - 1}, CellState::BLACK);      // (3, 2)
	set({c, c}, CellState::WHITE);          // (3, 3)
}
